...
import { StatePhysicaltargets } from 'src/app/Model/StatePhysicaltargtes';


@Component({
  selector: 'app-district-datatargets',
  templateUrl: './district-datatargets.component.html',
  styleUrls: ['./district-datatargets.component.css']
})
export class DistrictDatatargetsComponent {
  ...
  public dataSource = new MatTableDataSource<StatePhysicaltargets>();
...
input: any;
displayedColumns: string[] = ['slNo', 'Component_Code', 'Component', 'Unit', 'Physical_Target', 'Physical_Achievement','Financial_Target','GOIShare', 'GOIShareState', 'Progressive_FA' ,'GOIShareFA' , 'GOIShareStateFA' , 'SpillProgressive_PA', 'SpillProgressive_FA' , 'Action'];
...
this.adminservicesService.getmonths().subscribe((resultdistrictmaster) => {
  ...
  console.log(this.selectmonthmaster, 'months');
      ...
      GetDistrictwisetargets() {   
        this.adminservicesService.GetDistrictwisephsicaltargets(this.statecd, this.distcd, this.monthcd, this.yearcd).subscribe((data: any) => {
         
          this.dataSource.data = data as StatePhysicaltargets[];
          console.log( this.dataSource.data, 'SaveData');

        });
      }

      GetDistrictwiseTargetsdata()
      {
        this.adminservicesService.GetDistrictwisephsicaltargets(this.statecd, this.distcd, this.monthcd, this.yearcd).subscribe((result: any) => {
    
          this.data = result;
          console.log(this.data, 'Districttargets');
        });
